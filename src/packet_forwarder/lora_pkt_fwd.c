...
static char sx126x_dev_path[64];
static char i2c_dev_path[64];

int Sx126xBusyPin =  0xff;
int Sx126xResetPin = 0xff;
...
static int parse_SX130x_configuration(const char * conf_file) {
    ...
    str = json_object_get_string(conf_obj, "sx126x_path");
    if (str != NULL) {
        strncpy(sx126x_dev_path, str, sizeof sx126x_dev_path);
        sx126x_dev_path[sizeof sx126x_dev_path - 1] = '\0'; /* ensure string termination */
    } else {
        MSG("ERROR: spidev path must be configured in %s\n", conf_file);
        return -1;
    }
    str = json_object_get_string(conf_obj, "i2cdev_path");
    if (str != NULL) {
        strncpy(i2c_dev_path, str, sizeof i2c_dev_path);
        i2c_dev_path[sizeof i2c_dev_path - 1] = '\0'; /* ensure string termination */
    } else {
        MSG("ERROR: i2c_dev_path path must be configured in %s\n", conf_file);
        return -1;
    }
    MSG("INFO: i2c_dev_path %s\n", i2c_dev_path);
    val = json_object_get_value(conf_obj, "sx126x_busy_pin"); /* fetch value (if possible) */
    if (json_value_get_type(val) == JSONNumber) {
        Sx126xBusyPin = (int)json_value_get_number(val);
    } else {
        MSG("WARNING: Data type for sx126x_busy_pin seems wrong, please check\n");
        Sx126xBusyPin = 0xff;
    }
    val = json_object_get_value(conf_obj, "sx126x_reset_pin"); /* fetch value (if possible) */
    if (json_value_get_type(val) == JSONNumber) {
        Sx126xResetPin = (int)json_value_get_number(val);
    } else {
        MSG("WARNING: Data type for sx126x_reset_pin seems wrong, please check\n");
        Sx126xResetPin = 0xff;
    }
    MSG("INFO: Sx126xBusyPin %d, Sx126xResetPin %d\n", Sx126xBusyPin, Sx126xResetPin);
...
//int speed_arr[] = { B38400, B19200, B9600, B4800, B2400, B1200, B300,B38400, B19200, B9600, B4800, B2400, B1200, B300, }; //波特率 /*********************************/
...
void set_speed(int fd, int speed){  /*********************************/
  ...
  int status;   
  struct termios Opt;  
  tcgetattr(fd, &Opt);  // 将文件描述符fd的属性放入Opt结构体
  for (i= 0;i<sizeof(speed_arr)/sizeof(int);i++) {   
    if(speed==name_arr[i]){       
      tcflush(fd, TCIOFLUSH);//刷清输入、输出队列 
      cfsetispeed(&Opt,speed_arr[i]);    
      cfsetospeed(&Opt,speed_arr[i]);     
      status=tcsetattr(fd,TCSANOW,&Opt);//使用Opt的参数设置fd TCSANOW立即生效    
      if(status!=0) {          
...
void tolora(int sp0){/*********************************/
    ...
    while(true){
        ...
        int command_len=8;//comand length
        printf("reading from 485 \n");
        char readbuf[350];
        char *buf=(char*)malloc(command_len);
        int len=0;
        //pthread_rwlock_rdlock(&rwlock);
        len = read(sp0, &readbuf,command_len);
        //pthread_rwlock_unlock(&rwlock);
        if(len == command_len)
        {
            printf("########################Received data from 485:\n");
            for (int i = 0; i < len; i++)
            {
        ...
        {
            int index = command_len-len;
            while(index > 0)
            {
                
                char Buff[200] = {0};
                int len_back = read(sp0, Buff, sizeof(Buff));
                for(int i = 0; i < len_back; i++)
                {
                    readbuf[len+i] = Buff[i];
                    if(len+i+1==command_len)
                    {
                        break;
                    }
                }
        ...
        //int n = read(sp0, &readbuf, 10);
       /*
        printf("len=%d\n",n);
        if (n < 0) {
          printf("error reading from 485\n");
        } else {
            printf("########################Received data from 485:");
            for(int i=0;i<n;i++){
                printf("%x", readbuf[i]);
        ...
        */
        ...
        for(int i=0;i<n;i++){
            memcpy(buf+4+i, &readbuf[i], 1);
            }
        
        printf("buf to be sent:");
        for(int i =0;i<n+4;i++){
            printf("%x",buf[i]);
            }
        printf("\n");
        ...
        pkt.datarate = DR_LORA_SF10;//set the datarate 10
        ...
        printf("\n");                                                                                    
        int result = lgw_send(&pkt);
        if (result == LGW_HAL_ERROR) {
            MSG("WARNING: [TOLORA] lgw_send failed on rf_chain\n");
            continue;
        } else {
            MSG("[TOLORA] lgw_send done\n");
        }
        */
        ...
        struct lgw_pkt_tx_s pkt;
        pkt.freq_hz = 470300000;//change the freq
        pkt.tx_mode = IMMEDIATE;
        pkt.count_us = 1000;
        pkt.rf_chain = 0;
        pkt.rf_power = 22;
        ...
        pkt.no_crc = 1;
        pkt.no_header = 0;
        for(int count=0;count<command_len;count++){
            pkt.payload[count]=buf[count];
        }
        
        pkt.size=command_len;
        printf("pkt.payload:");
        for(int i =0;i<command_len;i++){
            printf("%x",pkt.payload[i]);
            }
        ...
        Radio.SetTxConfig(MODEM_LORA, 22, 0, BW_125KHZ, DR_LORA_SF10, CR_LORA_4_5, 0, false, true, 0, 0, 1, 10000);
        Radio.Send(pkt.payload, pkt.size);
        pthread_mutex_unlock(&mx_concent);
        printf("Radio Send Freq     :%d\n", 470000000);
        printf("Radio Send BandWidth:%d\n", BW_125KHZ);
        printf("Radio Send Sf       :%d\n", DR_LORA_SF10);
        printf("Radio Send Coderate :%d\n", CR_LORA_4_5);
        printf("Radio Send InvertPol:%d\n", 1);
...
int main(int argc, char ** argv)
{
    ...
    SX126xSpiInit(sx126x_dev_path);
    Radio.Init(NULL);
    Radio.SetMaxPayloadLength( MODEM_LORA, 250 );
    i = lgw_start(i2c_dev_path);
    ...
    struct serial_rs485 rs485conf_sp0={0};
    int n = 0;
    int sp0 = 0;
    struct termios tio;
    ...
    sp0 = open ("/dev/ttyUSB0", O_RDWR);
    if (sp0 < 0) {
       printf("Error: Can't open: /dev/ttyUSB0\n");
    }
    else{
            printf("OPEN TTYUSB0 successfully sp0 = %d\n",sp0);
        }
    ...
    i = pthread_create( &thrid_up, NULL, (void * (*)(void *))thread_up, (void *)sp0);
    printf("create thread thread-up successfully\n");
    //i = pthread_create( &thrid_up, NULL, (void (*)(int))thread_up, &fd);
    if (i != 0) {
    ...
    i = pthread_create( &thrid_tolora, NULL, (void * (*)(void *))tolora, (void *)sp0);/*********************************/
    //i = pthread_create( &thrid_tolora, NULL, (void (*)(int))tolora, &fd);
	if (i != 0) {
        MSG("ERROR: [main] impossible to create tolora thread\n");
        exit(EXIT_FAILURE);
    }
    ...
    while (!exit_sig && !quit_sig) {
        ...
        if (nb_pkt == LGW_HAL_ERROR) {
            ...
            exit(EXIT_FAILURE);
                ...
                //MSG("#########: [up] packet fetch done\n");
            }
        
         /*通过485发送packet*//*********************************/
         /*
		for(int i=0;i<nb_pkt;i++){
        	struct lgw_pkt_rx_s temp=rxpkt[i];
        	if(temp.size==0){
        		continue;
			}
			else{
				char *ch = (char*)malloc(sizeof(char)*sizeof(temp));
				memset(ch, 0, sizeof(ch));
				memcpy(ch, &temp, sizeof(temp));
				int res_write = write(fd,ch,sizeof(ch));
                if(res_write<0){
                    MSG("ERROR: [up] failed to WRITE DATA TO 485\n");
                    }
                else{
                        MSG("###########THREAD_UP: [up] WRITE DATA TO 485\n");
                        }
        ...
        */
        for(int i=0;i<nb_pkt;i++){
        	...
        	if(temp.size==0){
				...
				char *ch = (char*)malloc(sizeof(char)*temp.size);
                uint8_t pl[256];
                printf("Received packets from if_chain:%d,rf_chain:%d\n",temp.if_chain,temp.rf_chain);
                printf("payload size=%d",temp.size);
                for(int j=0;j<temp.size;j++){
                    pl[j]=temp.payload[j];
                    }
                printf("payload:");
                for(int j=0;j<temp.size;j++){
                    printf("%2x",temp.payload[j]);
                }
                ...
                /*
                if(temp.payload[0]==0x01&&temp.payload[1]==0x03&&temp.payload[2]==0x04&&temp.payload[3]==0x01){
                    
                    //pthread_rwlock_wrlock(&rwlock);
                    int res_write = write(fd,pl,temp.size);
                    tcflush(fd,TCOFLUSH);
                    //pthread_rwlock_unlock(&rwlock);
                    if(res_write<=0){
                        MSG("ERROR: [up] failed to WRITE DATA TO 485\n");
            ...
            if (jit_result == JIT_ERROR_OK) {
                if (pkt_index > -1) {
                    ...
                    if (jit_result == JIT_ERROR_OK) {
                        ...
                        {
                            case BW_125KHZ: LoRaBw = 0; break;
                            case BW_250KHZ: LoRaBw = 1; break;
                            case BW_500KHZ: LoRaBw = 2; break;
                            default:        LoRaBw = 0; break;
                        }
                        pthread_mutex_lock(&mx_concent);
                        Radio.SetPublicNetwork(true);
                        Radio.SetChannel(pkt.freq_hz);
                        Radio.SetTxConfig(MODEM_LORA, 22, 0, LoRaBw, pkt.datarate, pkt.coderate, pkt.preamble, false, true, 0, 0, pkt.invert_pol, 10000);
                        Radio.Send(pkt.payload, pkt.size);
                        pthread_mutex_unlock(&mx_concent);
                        printf("Radio Send Freq     :%d\n", pkt.freq_hz);
                        printf("Radio Send BandWidth:%d\n", LoRaBw);
